...
function rc_geo_access_restricted_countries_render() { 
    ...
    if ( $options[ 'rc_geo_access_status' ] === 'enabled' && ( $options[ 'rc_geo_access_ipstack_api_key' ] !== '' || $options[ 'rc_geo_access_ipgeolocation_api_key' ] !== '' ) ) {
        $rga_countries_whitelist_style = 'style="display:block;"';
        
    } else {
        
        $rga_countries_whitelist_style = 'style="display:none;"';
        echo '<p style="margin-bottom:50px;">' . __( 'Set Restriction Status to "Enabled" and enter an API Key above to enable the Countries Whitelist.', 'rc_geo_access_plugin' ) . '</p>';
...
function rc_geo_access_func() {
    ...
    if ( $rc_geo_status === 'enabled' && $rc_geo_access_api_key !== '' ) { // Check that Restriction is enabled and API Key is not empty...
